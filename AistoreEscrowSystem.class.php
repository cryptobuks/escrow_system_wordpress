...
?>
  <div>
    ...
    SET term_condition = '%s'  WHERE id = '%d'", $term_condition, $eid));
            ...
            $details_escrow_page_url = esc_url(add_query_arg(array('page_id' => get_option('details_escrow_page_id'), 'eid' => $eid,), home_url()));
?>
...
<div class="alert alert-success" role="alert">
 <?php printf(__('Successfully  with Escrow id %d.', 'aistore'), $eid); ?>
</div>
<br>
...
?>
    ...
    <div class="alert alert-success" role="alert">
 <?php printf(__('Successfully send payment with Escrow id %d.', 'aistore'), $eid); ?>
</div>
  ...
  <?php
            ...
            $settings = array('tinymce' => array('toolbar1' => 'bold,italic,underline,separator,alignleft,aligncenter,alignright   ', 'toolbar2' => '', 'toolbar3' => ''), 'textarea_rows' => 1, 'teeny' => true, 'quicktags' => false, 'media_buttons' => false);
...
?>
  <div>
      ...
      <?php
        ...
        if (isset($_POST['submit']) and $_POST['action'] == 'escrow_system') {
            ...
            if ($user_balance < $amount) {
                ...
                $create_escrow_page_2_url = esc_url(add_query_arg(array('page_id' => get_option('create_escrow_page_2'), 'eid' => $eid,), home_url()));
                // email to sender
                $to = $sender_email;
                $subject = $details;
     ...
     Invited to : " . $receiver_email . "<br>Escrow ID is:" . $eid . "<br>Accept Escrow system to :<br>" . $details_escrow_page_id_url;
     ...
     Invited by : " . $sender_email . "<br>Escrow ID is:" . $eid . "<br>Accept Escrow system to :<br>" . $details_escrow_page_id_url;
                $body.= "<br /> If you are not registered in the portal kindly visit my account page and register.";
                $headers = array('Content-Type: text/html; charset=UTF-8');
                wp_mail($to, $subject, $body, $headers);
?>
...
<div><?php
                printf(__('Escrow Fee %s.', 'aistore'), $escrow_fee); ?> </div>
...
<?php
    ...
    public static function aistore_escrow_list() {
        ...
        global $wpdb;
        $results = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}escrow_system WHERE sender_email=%s order by id desc limit 100", $current_user_email_id));
?>
...
?>
    ...
    <?php
            foreach ($results as $row):
                $details_escrow_page_id_url = esc_url(add_query_arg(array('page_id' => get_option('details_escrow_page_id'), 'eid' => $row->id,), home_url()));
?>
...
<?php
        $results = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}escrow_system WHERE receiver_email=%s order by id desc limit 100", $current_user_email_id));
        if ($results == null) {
            _e('Escrow List Not Found', 'aistore');
        } else {
?>
    ...
    <?php
            foreach ($results as $row):
                $details_escrow_page_url = esc_url(add_query_arg(array('page_id' => get_option('details_escrow_page_id'), 'eid' => $row->id,), home_url()));
?> 
      <tr>
    ...
    public static function aistore_escrow_detail() {
        ...
        ob_start();
        if (!isset($eid)) {
            $list_escrow_page_url = esc_url(add_query_arg(array('page_id' => get_option('list_escrow_page_id'),), home_url()));
?>
...
<?php wp_nonce_field('aistore_nonce_action', 'aistore_nonce'); ?>
  ...
  <?php
        $content = '';
        $editor_id = 'message';
        $settings = array('tinymce' => array('toolbar1' => 'bold,italic,underline,separator,alignleft,aligncenter,alignright, link,unlink,  ', 'toolbar2' => '', 'toolbar3' => ''), 'textarea_rows' => 1, 'teeny' => true, 'quicktags' => false, 'media_buttons' => false);
        wp_editor($content, $editor_id, $settings);
?>
...
<?php
        $discussions = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}escrow_discussion WHERE eid=%s order by id desc", $escrow->id));
?> 
...
<hr>
    ...
    function accept_escrow_btn($escrow) {
        ...
        else if ($escrow->status == "released") return "";
        else if ($escrow->status == "cancelled") return "";
        else if ($escrow->status == "disputed") return "";
        else if ($escrow->status == "accepted") return "";
        else if ($escrow->sender_email == $user_email) return "";
?>
...
<?php wp_nonce_field('aistore_nonce_action', 'aistore_nonce'); ?>
  ...
  <input type="hidden" name="action" value="cancelled" />
    ...
    function release_escrow_btn($escrow) {
        ...
        else if ($escrow->status == "pending") return "";
        if (!aistore_isadmin()) {
